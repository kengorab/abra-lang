...
impl Parser {
    ...
    fn parse_binding_decl(&mut self) -> Result<AstNode, ParseError> {
        ...
        let kind = match self.expect_next()? {
            tok @ Token::Ident(_, _) => BindingDeclKind::Variable(tok),
            Token::LParen(_, _) => {
                ...
                loop {
                    match self.expect_next()? {
                        ...
                        tok @ _ => {
                            return if done {
                                Err(ParseError::ExpectedToken(TokenType::RParen, tok))
                            } else {
        ...
        Ok(AstNode::BindingDecl(token, BindingDeclNode { kind, is_mutable, type_ann, expr }))
    }
    ...
    fn parse_match_case(&mut self) -> Result<MatchCase, ParseError> {
        ...
        if Token::get_ident_name(&ident) == "_" {
            ...
            return Ok(MatchCase { match_type: MatchCaseType::Wildcard(ident), case_binding, args: None });
        }
        ...
        let args = if let Some(Token::LParen(_, _)) = self.peek() {
            ...
            loop {
                match self.expect_next()? {
                    i @ Token::Ident(_, _) => {
                        if !item_expected {
                            return Err(ParseError::ExpectedToken(TokenType::RParen, i));
                        }
                    ...
                    t @ Token::RParen(_) => {
                        if args.is_empty() {
                            return Err(ParseError::ExpectedToken(TokenType::Ident, t));
                        }
                        break;
                    }
                    tok @ _ => return Err(ParseError::UnexpectedToken(tok.clone()))
        ...
        Ok(MatchCase { match_type, case_binding, args })
    }
...
mod tests {
    ...
    fn parse_binding_decls_no_assignment() -> TestResult {
        ...
        let expected = vec![
            ...
            AstNode::BindingDecl(
                ...
                BindingDeclNode {
                    kind: BindingDeclKind::Variable(ident_token!((2, 5), "abc")),
                    is_mutable: true,
                    type_ann: None,
                    expr: None,
                },
        ...
        let expected = vec![
            ...
            AstNode::BindingDecl(
                ...
                BindingDeclNode {
                    ...
                    expr: Some(Box::new(
                        AstNode::Tuple(
                            Token::LParen(Position::new(2, 14), false),
                            vec![
                                int_literal!((2, 15), 1),
                                int_literal!((2, 18), 2),
                            ]
            ...
            FunctionDeclNode {
                ...
                body: vec![
                    AstNode::BindingDecl(
                        ...
                        BindingDeclNode {
                            kind: BindingDeclKind::Variable(ident_token!((1, 18), "a")),
                            is_mutable: false,
                            type_ann: None,
    ...
    fn parse_indexing_separate_expressions() -> TestResult {
        ...
        let expected = vec![
            AstNode::BindingDecl(Token::Val(Position::new(1, 1)), BindingDeclNode {
                kind: BindingDeclKind::Variable(ident_token!((1, 5), "a")),
                type_ann: None,
    ...
    fn parse_if_expression() -> TestResult {
        ...
        let expected = AstNode::BindingDecl(
            Token::Val(Position::new(1, 1)),
            BindingDeclNode {
                kind: BindingDeclKind::Variable(ident_token!((1, 5), "str")),
                is_mutable: false,
                type_ann: None,
                expr: Some(Box::new(
    ...
    fn parse_match_statement() -> TestResult {
        ...
        let expected = AstNode::MatchStatement(
            ...
            MatchNode {
                ...
                branches: vec![
                    (
                        MatchCase { match_type: MatchCaseType::Ident(ident_token!((2, 1), "Int")), case_binding: None, args: None },
                    ...
                    (
                        MatchCase { match_type: MatchCaseType::Wildcard(ident_token!((6, 1), "_")), case_binding: None, args: None },
                        vec![int_literal!((6, 6), 0)]
                    ),
                    (
                        MatchCase { match_type: MatchCaseType::Wildcard(ident_token!((7, 1), "_")), case_binding: Some(ident_token!((7, 3), "x")), args: None },
